...
const BadLinks = () => {
    ...
    const getData = async () => {
        let result = await getServerAnalysisCurrentMonth('badLinks', page);
        if (result !== 'error') {
            return result;
        } else {
            throw new Error();
    ...
    const _onRefresh = async () => {
        setRefreshing(true);
        await queryClient.refetchQueries(['badLinks']);
        isMounted.current && setRefreshing(false);
    }

    const _onRemove = async () => {
        setRefreshing(true);
        await resolveServerAnalysisLastDays('badLinks', 1);
        await queryClient.refetchQueries(['badLinks']);
        isMounted.current && setRefreshing(false);
    }
        ...
        return (
            <div css={style.pageContainer}>
                <div css={style.container}>
                    <span css={style.title}> BadLinks </span>
                    --Error--
                </div>
            </div>
        );
